...
- [x] 3. 审计权限：先补默认授权、管理员保存、集中/单据页面及接口拒绝/允许测试；实现 `audit.view` 的 UI 与接口控制。
  ...
  > 用户验收修复：管理员角色权限在后端固定为全权限，原编辑卡片却允许取消勾选并提示保存成功。现从角色权限配置中移除管理员编辑卡片，接口直接拒绝修改管理员角色；其他角色的授权和撤权继续可用。补充 runtime、Prisma 与页面回归测试。

  > 修复验证：隔离 runtime 页面不再有管理员编辑卡片，销售角色仍可保存；直接修改管理员权限的 API 返回 400，前后记录一致。`make ci` 再次通过，shared 41、web 356、api 504，共 901 个测试。
