...
async function loadStockInList(
  ...
  try {
    ...
    const response = await fetch(`${getApiBaseUrl()}/stock-in?${params.toString()}`, {
      cache: 'no-store',
...
export default async function AppStockInPage({
  ...
  const stockInResult =
    (await loadStockInList(session, query)) ??
    paginateItems(fallbackItems, query.page, query.pageSize);
  const items = stockInResult.items;
    ...
    >
      <StatStrip
        items={[
          ...
          { label: '入库数量', value: totalQty },
        ]}
      />

      <section style={sectionStyle}>
        <section style={introCardStyle}>
          <h2 style={introTitleStyle}>收货入库说明</h2>
        ...
        <FormalDataTable title="收货单列表" total={stockInResult.total}>
          <div style={{ overflowX: 'auto' }}>
            <table style={tableStyle}>
              <thead>
                <tr>
                  <th style={headCellStyle}>收货单号</th>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td style={cellStyle}>
                      <Link href={`/app/stock-in/${item.id}`} style={rowLinkStyle}>
                        查看详情 {item.docNo}
                      </Link>
                    </td>
                  </tr>
          ...
          <FormalPagination
            ...
            page={stockInResult.page}
            pageSize={stockInResult.pageSize}
            total={stockInResult.total}
          />
        </FormalDataTable>
      </section>
    </AppShell>
