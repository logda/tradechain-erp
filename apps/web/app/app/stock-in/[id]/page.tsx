...
function resolveSourceHref(detail: StockDocumentDetail) {
  if (detail.sourceBizType === 'purchase_order') {
    return `/app/purchase-orders?docNo=${detail.sourceDocNo}`;
  }
...
async function loadDetail(id: string, session: { role: string; user: string }) {
  try {
    const response = await fetch(`${getApiBaseUrl()}/stock-in/${id}`, {
      cache: 'no-store',
...
export default async function AppStockInDetailPage({
  params,
  searchParams,
}: {
      ...
      <section style={sectionStyle}>
        <section style={sectionStyle}>
          <div style={sectionStyle}>
            <h3 style={sectionTitleStyle}>入库明细</h3>
          </div>
